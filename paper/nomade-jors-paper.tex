...
NØMADE addresses these gaps by combining zero-infrastructure deployment
...
Rather than treating each job independently, NØMADE models jobs as nodes
...
NØMADE uses cosine similarity on Z-score normalized feature vectors,
...
NØMADE provides: (1) zero-infrastructure deployment using a single
...
NØMADE includes an educational analytics module (\texttt{nomade edu}) designed
...
\textbf{Job Explanation} (\texttt{nomade edu explain}): Translates raw job data
...
\centering
\includegraphics[width=\textwidth]{figures/edu_explain.png}
\texttt{nomade edu explain 1104}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{figures/edu_trajectory.png}
\texttt{nomade edu trajectory alice}
\end{minipage}
...
\centering
\includegraphics[width=\textwidth]{figures/edu_report.png}
\texttt{nomade edu report cs101}
\end{minipage}

\caption{Educational analytics outputs. \textbf{Top left}: Job explanation
showing proficiency scores across four dimensions with actionable
recommendations. \textbf{Top right}: User trajectory tracking improvement over
173 jobs. \textbf{Bottom}: Group report aggregating proficiency data for a
course section, showing class-wide trends and per-student breakdown.}
\label{fig:edu}
\end{figure}

\textbf{User Trajectories} (\texttt{nomade edu trajectory}): Tracks an
...
\textbf{Group Reports} (\texttt{nomade edu report}): Aggregates proficiency
...
students. NØMADE automates this assessment while providing pedagogically useful
...
\textbf{Community Data Sharing}: The \texttt{nomade community export} command
...
\textbf{Educational Use}: Beyond the demo mode, NØMADE provides a complete
educational analytics pipeline. Instructors can assign Linux groups to course
sections (e.g., \texttt{cs101}, \texttt{bio301}) and use \texttt{nomade edu
report} to track class-wide proficiency development. Individual students
receive automated feedback via \texttt{nomade edu explain}, reducing instructor
